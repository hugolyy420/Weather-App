...
const APIManager = (() => {
  ...
  const formatDate = (tzid) => {
    ...
    const parsedDate = date_fns__WEBPACK_IMPORTED_MODULE_0__.parse(datePart, 'M/d/yyyy', new Date());
    const parsedTime = date_fns__WEBPACK_IMPORTED_MODULE_0__.parse(timePart, 'h:mm:ss a', new Date());
    const formattedDate = date_fns__WEBPACK_IMPORTED_MODULE_1__.format(parsedDate, 'EEEE dd MMMM yyyy');
    const formattedTime = date_fns__WEBPACK_IMPORTED_MODULE_1__.format(parsedTime, 'HH:mm');
    return { formattedDate, formattedTime };
  };

  const getDateWeekday = (dateString) => {
    const parsedDate = date_fns__WEBPACK_IMPORTED_MODULE_2__.parseISO(dateString);
    const weekday = date_fns__WEBPACK_IMPORTED_MODULE_1__.format(parsedDate, 'EEEE');
    return weekday;
  };
...
(function setEventListeners() {
  ...
  submitButton.addEventListener('click', async (event) => {
    ...
    try {
      const weatherData = await _API__WEBPACK_IMPORTED_MODULE_1__.APIManager.fetchWeatherData(placeValue);
      const weatherDataArray = [
        _API__WEBPACK_IMPORTED_MODULE_1__.APIManager.processCurrentWeatherData(weatherData),
        _API__WEBPACK_IMPORTED_MODULE_1__.APIManager.processForeCastWeatherData(weatherData),
      ];
