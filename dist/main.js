...
const dataProcessor = (() => {
  ...
  const formatDate = (tzid) => {
    ...
    const parsedDate = date_fns__WEBPACK_IMPORTED_MODULE_1__.parse(datePart, 'M/d/yyyy', new Date());
    const parsedTime = date_fns__WEBPACK_IMPORTED_MODULE_1__.parse(timePart, 'h:mm:ss a', new Date());
    const formattedDate = date_fns__WEBPACK_IMPORTED_MODULE_2__.format(parsedDate, 'EEEE dd MMMM yyyy');
    const formattedTime = date_fns__WEBPACK_IMPORTED_MODULE_2__.format(parsedTime, 'HH:mm');
    return { formattedDate, formattedTime };
  };

  const formateTime = (time) => {
    const parsedTime = date_fns__WEBPACK_IMPORTED_MODULE_1__.parse(time, 'hh:mm aa', new Date());
    const formattedTime = date_fns__WEBPACK_IMPORTED_MODULE_2__.format(parsedTime, 'HH:mm');
    return formattedTime;
  };

  const getDateWeekday = (dateString) => {
    const parsedDate = date_fns__WEBPACK_IMPORTED_MODULE_3__.parseISO(dateString);
    const weekday = date_fns__WEBPACK_IMPORTED_MODULE_2__.format(parsedDate, 'EEEE');
    return weekday;
  };

  const getWeatherIconClass = (weatherCode, isDaytime = true) => {
    const weather = _weatherIcons__WEBPACK_IMPORTED_MODULE_0__.weatherIcons.find((icon) =>
...
const weatherIcons = [
  { codes: [1000], icon: 'wi wi-day-sunny' },
  { codes: [1003], icon: 'wi wi-day-cloudy' },
  { codes: [1006, 1009], icon: 'wi wi-cloudy' },
  { codes: [1030, 1135, 1147], icon: 'wi wi-fog' },
  { codes: [1063, 1171, 1180, 1183, 1186, 1189, 1198], icon: 'wi wi-day-rain' },
  { codes: [1192, 1195, 1201, 1246], icon: 'wi wi-rain' },
  { codes: [1066, 1210, 1213, 1255], icon: 'wi wi-day-snow' },
  {
    codes: [1069, 1072, 1150, 1153, 1168, 1204, 1249],
    icon: 'wi wi-day-sleet',
  },
  { codes: [1087, 1273, 1276], icon: 'wi wi-thunderstorm' },
  { codes: [1114, 1117], icon: 'wi wi-snow-wind' },
  { codes: [1207, 1252], icon: 'wi wi-sleet' },
  { codes: [1237, 1261, 1264], icon: 'wi wi-snowflake-cold' },
  { codes: [1216, 1219, 1222, 1225, 1258], icon: 'wi wi-snow' },
  { codes: [1240], icon: 'wi wi-day-showers' },
  { codes: [1243], icon: 'wi wi-showers' },
  { codes: [1279, 1282], icon: 'wi wi-day-snow-thunderstorm' },
];
...
(function setEventListeners() {
    ...
    temperatureDataContainers.forEach((container) => {
      ...
      if (temperatureUnitToggler.classList.contains('celsius')) {
        const formattedTemperatureData =
          _dataProcessor__WEBPACK_IMPORTED_MODULE_1__.dataProcessor.celsiusToFahrenheit(temperatureData);
        temperatureDataContainer.textContent = `${formattedTemperatureData}°F`;
      } else {
        const formattedTemperatureData =
          _dataProcessor__WEBPACK_IMPORTED_MODULE_1__.dataProcessor.fahrenheitToCelsius(temperatureData);
  ...
  async function displayWeatherinfo(event) {
    _UI__WEBPACK_IMPORTED_MODULE_2__.domController.showLoadingSpinner();
    event.preventDefault();
    _UI__WEBPACK_IMPORTED_MODULE_2__.domController.hideErrorMessage();
    const placeValue = searchBar.value;
    try {
      if (placeValue === '') {
        throw new Error('Please do not search with blank value');
      }
      const weatherData = await _API__WEBPACK_IMPORTED_MODULE_3__.APIManager.fetchWeatherData(placeValue);
      ...
      const weatherDataArray = [
        _dataProcessor__WEBPACK_IMPORTED_MODULE_1__.dataProcessor.processCurrentWeatherData(weatherData),
        _dataProcessor__WEBPACK_IMPORTED_MODULE_1__.dataProcessor.processForeCastWeatherData(weatherData),
      ];
      _UI__WEBPACK_IMPORTED_MODULE_2__.domController.renderWeatherInfo(weatherDataArray);
      _UI__WEBPACK_IMPORTED_MODULE_2__.domController.hideLoadingSpinner();
    } catch (err) {
      _UI__WEBPACK_IMPORTED_MODULE_2__.domController.displayErrorMessage(
